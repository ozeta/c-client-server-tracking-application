...
Package * pkg_initialize(char *buffer0, char *buffer1, char *buffer2){
	...
	newPkg->next = NULL;
	if (pthread_mutex_init(&newPkg->m_lock, NULL) != 0) { 
		free(newPkg); 
    	write (STDERR_FILENO, err01, strlen (err01));
		return NULL; 
    }
	return newPkg;
}
...
Package * pkg_enqueue (Package * handler, char *buffer0, char *buffer1, char *buffer2) {
    if (handler == NULL){
        Package * nuovo_nodo = pkg_initialize (buffer0, buffer1, buffer2);
        if (nuovo_nodo != NULL) {
...
Package * pkg_push (Package * handler, char *buffer0, char *buffer1, char *buffer2) {
	Package * newPkg = pkg_initialize (buffer0, buffer1, buffer2);
	if (handler != NULL)
		newPkg->next = handler;
	return newPkg;
}
...
*/
		...
		//handler = pkg_enqueue (handler, string[0], string[1], string[2]);
		/**implementazione su pila*/
		handler = pkg_push (handler, string[0], string[1], string[2]);
